...
def execution_time(func: Callable[..., Any], n_samples: int = 10) -> float:
    times: list[float] = [0] * n_samples
...
def read_input(filename: str = "input.txt", to_int: bool = False) -> list[list[int]] | list[str]:
    ...
    with open(filename) as file:
        for line in file:
            if to_int:
                stripped = line.strip()
                stripped_int = [int(c) for c in stripped]
                lines.append(stripped_int)
            else:
                lines.append(line.strip())
    return lines


def save_output(file_path: str = "../README.md", new_row: list[float] = None) -> None:
    if new_row is None:
        new_row = [0.0, 0.0]
...
def plot_graph(G, path=None) -> None:
